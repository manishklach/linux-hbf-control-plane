...
/*
 * sequential-access.c  —  Benchmark HBF prefetch on sequential read
 *
 * Allocates a large buffer on the warm NUMA node, submits a PREFETCH hint,
 * then reads every page sequentially while measuring fault latency.
 *
 * Usage:
 *   ./sequential-access <size_mb> [hot_node] [warm_node]
 *
 * Output: JSON with fault counts, latency percentiles, and hint statistics.
 */
...
int main(int argc, char **argv)
{
	...
	struct hbf_user_hint hint = {
		...
		.len = size,
		.target_nid = hot_node,
		.deadline_ns = time_ns() + 500000000, // 500ms deadline
		.user_tag = 1,
	};
	...
	// Touch every page, measure latency
	...
	printf("{\n");
	printf("  \"benchmark\": \"sequential-access\",\n");
	printf("  \"size_mb\": %zu,\n", size_mb);
	printf("  \"hot_node\": %d,\n", hot_node);
	printf("  \"warm_node\": %d,\n", warm_node);
